		...
		try {
			...
			const res = await fetch('/api/admin/invoices', {
				...
				body: JSON.stringify({
					...
					total,
					balance_due: total,
					gst_type: formData.gst_type || 'cgst_sgst',
				}),
			});
					...
					<div className="flex justify-end">
						<div className="w-full max-w-sm space-y-3 bg-gray-50 rounded-lg p-4">
							...
							<div className="flex items-center justify-between text-sm">
								...
								<select
									value={formData.gst_type || 'cgst_sgst'}
									onChange={(e) =>
										setFormData({ ...formData, gst_type: e.target.value })
									}
									className="w-32 px-1 py-0.5 text-xs border border-gray-300 rounded focus:ring-2 focus:ring-purple-500 focus:border-transparent bg-white"
								>
							...
							<div className="flex items-center justify-between text-sm">
								...
								<div className="flex items-center">
									<input
										...
										className="w-16 px-2 py-1 border border-gray-300 rounded text-right focus:ring-2 focus:ring-purple-500 focus:border-transparent"
									/>
									<span className="ml-1 text-xs">%</span>
								</div>
							</div>
							{(formData.gst_type || 'cgst_sgst') === 'cgst_sgst' ? (
								<>
									<div className="flex justify-between text-xs text-gray-500 pl-2">
										<span>
											CGST @ {((formData.tax_rate || 0) / 2).toFixed(1)}%:
										</span>
										<span className="font-medium">
											{formatCurrency((calculateTotals().taxAmount || 0) / 2)}
										</span>
									</div>
									<div className="flex justify-between text-xs text-gray-500 pl-2">
										<span>
											SGST @ {((formData.tax_rate || 0) / 2).toFixed(1)}%:
										</span>
										<span className="font-medium">
											{formatCurrency((calculateTotals().taxAmount || 0) / 2)}
										</span>
								...
								<div className="flex justify-between text-xs text-gray-500 pl-2">
									<span>IGST @ {formData.tax_rate || 0}%:</span>
									<span className="font-medium">
