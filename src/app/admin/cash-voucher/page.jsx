...
export default function CashVoucherPage() {
	...
	const handleDelete = async (id) => {
		...
		try {
			const res = await fetch(`/api/admin/cash-vouchers?id=${id}`, {
				method: 'DELETE',
			});
			const data = await res.json();
											...
											>
												...
												<td className="px-6 py-4">
													<div className="font-medium text-gray-900">
														{voucher.paid_to || voucher.payee_name || '-'}
													</div>
												</td>
												<td className="px-6 py-4 text-gray-600 max-w-[200px] truncate">
													{voucher.prepared_by || voucher.description || '-'}
												</td>
												<td className="px-6 py-4 font-semibold text-gray-900">
													{formatCurrency(
														voucher.total_amount || voucher.amount
													)}
												</td>
												<td className="px-6 py-4">
													<div className="flex items-center justify-center gap-1">
														<button
															onClick={() =>
																window.open(
																	`/api/admin/cash-vouchers/download?id=${voucher.id}`,
																	'_blank'
																)
															}
