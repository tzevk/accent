...
export default function ActivityLogsPage() {
	...
	const [workLogsPagination, setWorkLogsPagination] = useState({
		page: 1,
		limit: 50,
		total: 0,
		totalPages: 0,
	...
	const fetchAuditLogs = async () => {
		...
		try {
			...
			if (auditResource !== 'all')
				params.append('resource_type', auditResource);
								...
								<div className="flex items-center justify-between mb-4">
									<div className="flex items-center gap-3">
										<div className="w-12 h-12 bg-gradient-to-br from-[#64126D] to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-lg">
											{users
												.find((u) => u.id === parseInt(selectedUser))
											...
											<div className="flex items-center gap-2 mt-1">
												{userStatusMap[selectedUser] && (
													<StatusBadgeWithBg
														...
														lastActivity={
															userStatusMap[selectedUser].last_activity
														}
													/>
												)}
												{userStatusMap[selectedUser]?.current_page &&
													userStatusMap[selectedUser].status === 'online' && (
														<span className="text-xs text-gray-500">
															📍{' '}
															{userStatusMap[selectedUser].current_page.replace(
																/^\//,
																''
															)}
														</span>
								...
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
									<div className="bg-blue-50 rounded-lg p-4">
										...
										<p className="text-2xl font-bold text-blue-600">
											{workSummary.total_activities || 0}
										</p>
										<p className="text-xs text-blue-700 mt-1">
											{workSummary.unique_action_types || 0} types
										</p>
									</div>
									...
									<div className="bg-green-50 rounded-lg p-4">
										...
										<p className="text-2xl font-bold text-green-600">
											{workSummary.pages_visited || 0}
										</p>
										<p className="text-xs text-green-700 mt-1">
											{workSummary.unique_pages || 0} unique
										</p>
									</div>
									...
									<div className="bg-purple-50 rounded-lg p-4">
										...
										<p className="text-2xl font-bold text-purple-600">
											{workSummary.active_hours || '0'}h
										</p>
										<p className="text-xs text-purple-700 mt-1">
											{workSummary.active_percentage || 0}% active
										</p>
									</div>
									...
									<div className="bg-orange-50 rounded-lg p-4">
										...
										<p className="text-2xl font-bold text-orange-600">
											{workSummary.work_items_touched || 0}
										</p>
										<p className="text-xs text-orange-700 mt-1">
											{workSummary.creates || 0} created,{' '}
											{workSummary.updates || 0} updated
										</p>
									</div>
								</div>

								{workSummary.recent_pages &&
									workSummary.recent_pages.length > 0 && (
										...
										{loading ? (
											...
											filteredLogs.map((log) => (
												<tr key={log.id} className="hover:bg-gray-50">
													...
													<td className="px-6 py-4 whitespace-nowrap">
														<div className="flex items-center">
															<div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center mr-3 relative">
																...
																{userStatusMap[log.user_id]?.status ===
																	'online' && (
																	<span
																		className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 border-2 border-white rounded-full animate-pulse"
																		title="Online Now"
																	></span>
																)}
															</div>
															<div>
																...
																<div className="flex items-center gap-2">
																	...
																	{userStatusMap[log.user_id] && (
																		<StatusBadge
																			status={userStatusMap[log.user_id].status}
																		/>
																	)}
																</div>
															</div>
													...
													<td className="px-6 py-4 whitespace-nowrap">
														<span
															className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium ${getActionColor(log.action_type)}`}
														>
																	...
																	<div className="mt-1 space-y-1">
																		...
																		{log.details.durationMinutes && (
																			<div className="text-xs text-purple-600">
																				⏱️ {log.details.durationMinutes} min
																			</div>
																		)}
																		{log.details.project && (
																			<div className="text-xs text-green-600">
																				📁 Project: {log.details.project}
																			</div>
																		)}
																		{log.details.lead && (
																			<div className="text-xs text-yellow-600">
																				👤 Lead: {log.details.lead}
																			</div>
																		)}
																		{log.details.status && (
							...
							{!workLogsLoading && workLogs.length > 0 && (
								<div className="bg-gray-50 px-6 py-4 flex items-center justify-between border-t">
									<div className="text-sm text-gray-700">
										Showing{' '}
										<span className="font-medium">
											{(workLogsPagination.page - 1) *
												workLogsPagination.limit +
												1}
										</span>{' '}
										to{' '}
										<span className="font-medium">
											{Math.min(
												workLogsPagination.page * workLogsPagination.limit,
												workLogsPagination.total
											)}
										</span>{' '}
										of{' '}
										<span className="font-medium">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
								...
								<div>
									...
									>
										...
										{Object.entries(AUDIT_RESOURCE_LABELS).map(
											([value, label]) => (
												<option key={value} value={value}>
													{label}
												</option>
											...
											auditLogs.map((log, index) => (
												<React.Fragment key={log.id}>
													<tr className="hover:bg-gray-50">
														...
														<td className="px-6 py-4 whitespace-nowrap">
															<span
																className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${AUDIT_ACTION_COLORS[log.action] || 'bg-gray-100 text-gray-800'}`}
															>
															...
															{log.old_values || log.new_values ? (
																...
																>
																	{expandedAuditRow === log.id
																		...
																		: 'View'}
																	<ChevronDownIcon
																		className={`w-4 h-4 transition-transform ${expandedAuditRow === log.id ? 'rotate-180' : ''}`}
																	/>
																</button>
															) : (
							...
							{!auditLogsLoading && auditLogs.length > 0 && (
								<div className="bg-gray-50 px-6 py-4 flex items-center justify-between border-t">
									<div className="text-sm text-gray-700">
										...
										<span className="font-medium">
											{Math.min(
												auditLogsPagination.page * auditLogsPagination.limit,
