	...
	const fetchQuotation = useCallback(async () => {
		...
		try {
			...
			if (data.success && data.data) {
				...
				let scopeItems = [
					{ sr_no: 1, description: '', qty: '', rate: '', amount: '' },
				];
				if (q.scope_items) {
					try {
						scopeItems =
				...
				setQuotation({
					...
					terms_and_conditions:
						q.terms_and_conditions || quotation.terms_and_conditions,
					...
					annexure_codes_standards:
						q.annexure_codes_standards || quotation.annexure_codes_standards,
					annexure_dispute_resolution:
						q.annexure_dispute_resolution ||
	...
	const addScopeItem = () => {
		setQuotation((prev) => ({
			...
			scope_items: [
				...prev.scope_items,
				{
					sr_no: prev.scope_items.length + 1,
					description: '',
					qty: '',
					rate: '',
					amount: '',
	...
	const saveQuotation = async () => {
		...
		try {
			...
			if (source === 'quotations') {
				...
				url = isNew
					? `/api/admin/quotations?source=${source}`
					: `/api/admin/quotations/${id}?source=${source}`;
			}
						...
						<div className="flex items-center gap-4">
							...
							<div>
								<h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
									...
									{isNew ? 'Create Standalone Quotation' : 'Edit Quotation'}
								</h1>
								{!isNew && (
								...
								<div className="flex">
									<div className="flex-1 p-4 border-r border-gray-300">
										<p className="text-sm text-gray-600">
											Receivers Signature with Company Seal.
										</p>
