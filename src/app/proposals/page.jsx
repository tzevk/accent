...
export default function Proposals() {
	...
	const canViewProposals =
		!rbacLoading && can(RBAC_RESOURCES.PROPOSALS, RBAC_PERMISSIONS.READ);
	...
	const handleConvert = async (proposal) => {
		try {
			...
			if (
				!window.confirm(
					`Convert proposal "${proposal.proposal_title || proposal.title}" to a project?`
				)
			)
				return;
				...
				prev.map((p) =>
					p.id === proposal.id
						? {
								...p,
								status: 'CONVERTED',
								project_id: data.data.project.project_id,
							}
						: p
				)
			);
			...
			if (!targetId) {
				alert(
					'Project created but no redirect id returned. Open Projects list to view it.'
				);
			} else {
	...
	const exportAllProposals = async () => {
		try {
			...
			showToast(
				`✅ Exported ${proposals.length} proposals to PDF: ${filename}`
			);
		} catch (err) {
	...
	if (!rbacLoading && !canViewProposals) {
		return (
		...
		<div className="h-screen bg-gray-50 flex flex-col overflow-hidden">
			...
			<div className="flex-shrink-0 pt-24 px-6 lg:px-8 xl:px-12 2xl:px-16 pb-4 max-w-[1920px] mx-auto w-full">
				...
				<div className="flex border-b border-gray-200 bg-white rounded-t-lg px-6 pt-4">
					<button className="px-6 py-3 text-sm font-medium border-b-2 border-black text-black bg-gray-50 rounded-t-md transition-colors">
											...
											<tr>
												<td colSpan="6" className="px-6 py-12 text-center">
													...
													<p className="mt-1 text-sm text-gray-500">
														Proposals are generated from leads using the
