...
export async function GET(request, { params }) {
  ...
  try {
    ...
    if (structures.length > 0) {
      ...
      structuresSheet.addRow({
        message: 'No salary structures found for this employee',
      });
      structuresSheet.getRow(1).font = { bold: true };
    }
    ...
    if (salaryProfiles.length > 0) {
      ...
      profileSheet.addRow({
        message: 'No employee salary profile records found for this employee',
      });
      profileSheet.getRow(1).font = { bold: true };
    }
    ...
    if (components.length > 0) {
      ...
      componentsSheet.addRow({
        message: 'No salary structure components found for this employee',
      });
      componentsSheet.getRow(1).font = { bold: true };
    }
    ...
    const safeCode = String(
      employee.employee_id || `EMP_${employee.id}`
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to export salary structure',
