...
export async function POST(request) {
  ...
  let db;
  try {
    const { function_id, activity_name, default_manhours } =
      await request.json();
...
export async function DELETE(request) {
  ...
  try {
    ...
    if (!id && !functionId) {
      return NextResponse.json(
        { success: false, error: 'Activity ID or Function ID is required' },
        { status: 400 }
      );
    }
    ...
    if (functionId) {
      ...
      const [activities] = await db.execute(
        'SELECT id FROM activities_master WHERE function_id = ?',
        [functionId]
      );

      // Delete sub-activities for all these activities
      for (const activity of activities) {
        await db.execute('DELETE FROM sub_activities WHERE activity_id = ?', [
          activity.id,
        ]);
      }
      ...
      const [result] = await db.execute(
        'DELETE FROM activities_master WHERE function_id = ?',
        [functionId]
      );

      return NextResponse.json({
        success: true,
        message: `${result.affectedRows} activities deleted successfully`,
      });
    } else {
      ...
      return NextResponse.json({
        success: true,
        message: 'Activity and its sub-activities deleted successfully',
      });
    }
  } catch (error) {
