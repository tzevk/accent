...
export async function POST(request) {
	...
	try {
		...
		// Build project payload from proposal - generate project_code in format serial_MM_YYYY
		const now = new Date();
		const month = String(now.getMonth() + 1).padStart(2, '0');
		const year = now.getFullYear();

		// Find highest serial from project_code (continues across months)
		...
		let maxSerial = 0;
		if (existingProjects.length > 0 && existingProjects[0].serial_num) {
			maxSerial = existingProjects[0].serial_num;
		}
		...
		// Insert project with project_code (project_id is auto-generated integer)
