	...
	try {
		...
		try {
			const [rows] = await db.execute(
				'SELECT * FROM proposal_approvals WHERE proposal_id = ? ORDER BY created_at DESC',
				[id]
			);
...
export async function POST(request, { params }) {
	...
	try {
		...
		try {
			...
			return NextResponse.json(
				{ success: true, message: 'Approval recorded' },
				{ status: 201 }
			);
		} catch {
			return NextResponse.json(
				{ success: true, message: 'Approval accepted (no backing table)' },
				{ status: 201 }
			);
		} finally {
			if (db) db.release();
		...
		return NextResponse.json(
			{ success: false, error: 'Failed to save approval' },
			{ status: 500 }
		);
	}
}
