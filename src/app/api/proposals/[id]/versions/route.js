	...
	try {
		...
		try {
			const [rows] = await db.execute(
				'SELECT * FROM proposal_versions WHERE proposal_id = ? ORDER BY created_at DESC',
				[id]
			);
	...
	try {
		...
		try {
         ...
         VALUES (?, ?, ?, ?, ?, ?, NOW())`,
			...
			return NextResponse.json(
				{ success: true, message: 'Version recorded' },
				{ status: 201 }
			);
		} catch {
			...
			return NextResponse.json(
				{ success: true, message: 'Version accepted (no backing table)' },
				{ status: 201 }
			);
		} finally {
			if (db) db.release();
