  ...
  try {
    ...
    try {
      const [rows] = await db.execute(
        'SELECT id, company_id, company_name, city, contact_person, industry, email, phone FROM companies ORDER BY company_name ASC'
      );
      companies = rows;
    } catch {
    ...
    try {
      const [rows] = await db.execute(
        "SELECT id, vendor_name, contact_person, email, phone, city, gst_vat_tax_id, status FROM vendors WHERE status != 'deleted' ORDER BY vendor_name ASC"
      );
      vendors = rows;
    } catch {
      try {
        const [rows] = await db.execute(
          "SELECT id, vendor_name, contact_person, email, phone, city, status FROM vendors WHERE status != 'deleted' ORDER BY vendor_name ASC"
        );
        vendors = rows;
      } catch {
        /* table may not exist */
    ...
    try {
      const [rows] = await db.execute(
        'SELECT id, activity_id, name, description, status FROM sub_activities ORDER BY name ASC'
      );
      subActivities = rows;
    } catch {
      /* table may not exist */
    ...
    try {
      ...
      console.warn(
        'init-data: Users JOIN query failed, trying simple query:',
        usersError.message
      );
      ...
      try {
        ...
        console.error(
          'init-data: Simple users query also failed:',
          simpleFail.message
        );
      }
    }
    ...
    try {
      const [cats] = await db.execute(
        'SELECT id, name, description, status FROM software_categories ORDER BY name'
      );
      ...
      const [vers] = await db.execute(
        'SELECT id, software_id, name, release_date, notes FROM software_versions ORDER BY name'
      );
    ...
    try {
      const [rows] = await db.execute(
        'SELECT id, doc_key, name, status, description FROM documents_master ORDER BY name ASC'
      );
      docMaster = rows;
    } catch {
      /* table may not exist */
    ...
    const softwareMaster = softwareCategories.map((cat) => ({
      ...
      softwares: softwares
        ...
        .map((sw) => ({
          ...sw,
          versions: softwareVersions.filter(
            (v) => String(v.software_id) === String(sw.id)
          ),
        })),
    }));
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to fetch init data',
