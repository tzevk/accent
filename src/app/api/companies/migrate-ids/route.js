...
async function buildMigrationPlan(db) {
  ...
  const [allIds] = await db.execute(`SELECT company_id FROM companies WHERE company_id IS NOT NULL AND company_id != ''`);
  ...
  let counter = maxRow && maxRow.length > 0 ? parseInt(maxRow[0].company_id.replace('COM-', ''), 10) : 0;
  ...
  for (const row of rows) {
    do { counter++; } while (takenIds.has(`COM-${String(counter).padStart(5, '0')}`));
    const newId = `COM-${String(counter).padStart(5, '0')}`;
    takenIds.add(newId); // reserve it for subsequent iterations
    plan.push({ id: row.id, old_company_id: row.company_id, company_name: row.company_name, new_company_id: newId });
  }
...
export async function POST(request) {
  ...
  try {
    ...
    if (plan.length === 0) {
      return NextResponse.json({ success: true, updated: 0, message: 'All companies already have valid COM-XXXXX IDs — nothing to do.' });
    }

    await db.beginTransaction();
    try {
      for (const item of plan) {
        await db.execute(`UPDATE companies SET company_id = ? WHERE id = ?`, [item.new_company_id, item.id]);
      }
      await db.commit();
    } catch (err) {
      await db.rollback();
