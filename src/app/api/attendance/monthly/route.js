		...
		const calcPayable =
			payable_days != null
				...
				: parseFloat(present_days) +
					parseFloat(half_days) * 0.5 +
					parseFloat(privileged_leave) +
					parseFloat(sick_leave) +
					parseFloat(casual_leave);
...
export async function PUT(request) {
	...
	try {
		...
		if (!id && !(employee_id && month && year)) {
			return NextResponse.json(
				{ error: 'Either id or (employee_id + month + year) is required' },
				{ status: 400 }
			);
		}
	...
	try {
		...
		if (id) {
			[result] = await connection.execute(
				'DELETE FROM attendance_monthly WHERE id = ?',
				[id]
			);
		} else {
			[result] = await connection.execute(
