...
export async function PUT(request) {
      ...
      `);
		...
		if (
           ...
           VALUES (?, ?, ?, ?, ?, ?, 'active')`,
				...
				await db.execute(`UPDATE users SET employee_id = ? WHERE id = ?`, [
					ins.insertId,
					currentUser.id,
				]);
					...
					try {
						...
						if (rows && rows.length > 0) {
							...
							await db.execute(
								...
								[existingId, currentUser.id]
							);
							currentUser.employee = { id: existingId };
						}
					} catch (linkErr) {
						console.warn(
							'Failed to link existing employee by email:',
							linkErr?.code || linkErr?.message || linkErr
						);
					}
				} else {
					console.warn(
						'Auto-create employee failed:',
		...
		if (wantsProfileUpdate) {
			...
			const fullName = normalizeString(profileChanges.full_name, {
				allowEmpty: false,
			});
			...
			if (typeof profileChanges.email !== 'undefined') {
				const email = normalizeString(profileChanges.email, {
					allowEmpty: false,
				});
				if (!email) {
					return NextResponse.json(
		...
		if (wantsEmployeeUpdate && currentUser.employee?.id) {
			...
			const processField = (column) => {
				if (!Object.prototype.hasOwnProperty.call(employeeChanges, column))
					return;
				...
				try {
					...
					if (error?.code === 'ER_DUP_ENTRY') {
						return NextResponse.json(
							{
								success: false,
								error: 'Duplicate employee information detected.',
							},
							{ status: 409 }
						);
					}
	...
	if (!refreshedUser) {
		return NextResponse.json(
			{ success: false, error: 'Unable to reload profile' },
			{ status: 500 }
		);
	}
	...
	return NextResponse.json({
		success: true,
		data: serializeResponse(refreshedUser),
	});
}
