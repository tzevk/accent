  ...
  try {
    ...
    try {
      const [tables] = await db.execute(
        `SHOW TABLES LIKE 'user_activity_assignments'`
      );
      if (tables.length > 0) {
      ...
      projects.map(async (project) => {
        ...
        if (useActivityAssignments) {
          ...
          `,
            ...
            console.log(
              'user_activity_assignments activities query failed:',
              err.message
            );
          }
        }
        ...
        if (activities.length === 0) {
          ...
          `,
            ...
            if (projectRow.length > 0 && projectRow[0].team_members) {
              const teamMembers =
                typeof projectRow[0].team_members === 'string'
                  ? JSON.parse(projectRow[0].team_members)
                  : projectRow[0].team_members;

              if (Array.isArray(teamMembers)) {
                // Find activities assigned to this user
                const userActivities = teamMembers.filter(
                  (tm) =>
                    tm.employee_id === requestedUserId ||
                    tm.user_id === requestedUserId
                );
        ...
        // Calculate totals
        const totalAssignedHours = activities.reduce(
          (sum, a) => sum + (parseFloat(a.assigned_manhours) || 0),
          0
        );
        const totalActualHours = activities.reduce(
          (sum, a) => sum + (parseFloat(a.actual_manhours) || 0),
          0
        );
    ...
    const overallStats = {
      ...
      totalActivities: projectsWithActivities.reduce(
        (sum, p) => sum + p.activityCount,
        0
      ),
      totalAssignedHours: projectsWithActivities.reduce(
        (sum, p) => sum + p.totalAssignedHours,
        0
      ),
      totalActualHours: projectsWithActivities.reduce(
        (sum, p) => sum + p.totalActualHours,
        0
      ),
      completedActivities: projectsWithActivities.reduce(
        (sum, p) => sum + p.completedCount,
        0
      ),
      inProgressActivities: projectsWithActivities.reduce(
