	...
	try {
      ...
      updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
		...
		const [cats] = await db.execute(
			'SELECT id, name, description, status, created_at, updated_at FROM software_categories ORDER BY name'
		);
		let softwares = [];
		try {
...
export async function PUT(request) {
	...
	try {
		...
		db = await dbConnect();
		await db.execute(
			`UPDATE software_categories SET name = COALESCE(?, name), description = COALESCE(?, description), status = COALESCE(?, status) WHERE id = ?`,
...
export async function DELETE(request) {
	...
	try {
		...
		try {
			await db.execute(
				'DELETE FROM software_versions WHERE software_id IN (SELECT id FROM softwares WHERE category_id = ?)',
				[id]
			);
		} catch {
