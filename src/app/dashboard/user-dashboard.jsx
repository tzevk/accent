...
const ProjectActivityAssignments = lazy(
  () => import('@/components/ProjectActivityAssignments')
);
...
export default function UserDashboard({ verifiedUser }) {
  ...
  const [liveTotalTime, setLiveTotalTime] = useState({
    hrs: 0,
    mins: 0,
    elapsedHrs: 0,
    elapsedMins: 0,
  ...
  useEffect(() => {
    const calcTotal = () => {
      ...
      const now = new Date();
      const [lh, lm] = (typeof lt === 'string' ? lt : lt.toString())
        .split(':')
        .map(Number);
      ...
      if (attendance.logoutTime) {
        const [oh, om] = (
          typeof attendance.logoutTime === 'string'
            ...
            : attendance.logoutTime.toString()
        )
          .split(':')
          .map(Number);
        endDate = new Date(
      ...
      setLiveTotalTime({
        hrs: Math.floor(netMin / 60),
        mins: netMin % 60,
        elapsedHrs: Math.floor(elapsedMin / 60),
        elapsedMins: elapsedMin % 60,
        idleHrs: Math.floor(idleMin / 60),
        idleMins: idleMin % 60,
      });
    };
    calcTotal();
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [attendanceRes, activityRes] = await Promise.allSettled([
          fetchJSON(`/api/users/${userId}/attendance`, { signal }).catch(
            () => null
          ),
          fetch(`/api/users/${userId}/activity-assignments`, { signal })
            .then((r) => r.json())
            .catch(() => null),
        ]);
        ...
        const tier2 = await Promise.allSettled([
          fetch('/api/messages/unread-count', { signal })
            .then((r) => {
              if (r.status === 401 || r.status === 403)
                return { _denied: 'messages' };
              return r.json();
            })
        ...
        else if (msgVal?.success)
          dataPatch.unreadMessages = msgVal.data.unread_count || 0;
        ...
        if (todoVal?._denied) accessPatch.todos = false;
          ...
          dataPatch.highPriorityTodos = pending.filter(
            (t) => t.priority === 'high'
          ).length;
        }
        ...
        if (wlVal?._denied) accessPatch.workLogs = false;
          ...
          const totalHours = wlVal.data.reduce(
            (sum, log) => sum + ((log && parseFloat(log.hours_worked)) || 0),
            0
          );
        ...
        if (Object.keys(dataPatch).length)
          setModuleAssignments((prev) => ({ ...prev, ...dataPatch }));
        if (Object.keys(accessPatch).length)
          setModuleAccess((prev) => ({ ...prev, ...accessPatch }));
      } catch (error) {
            ...
            <div className="px-6 py-4">
              <p className="text-sm text-gray-700 mb-3">
                ...
                {pendingActivities.length === 1
                  ? 'activity has'
                  : 'activities have'}{' '}
                not been updated today. Please submit your daily progress.
              </p>
              <div className="max-h-48 overflow-y-auto space-y-1.5">
                {pendingActivities.slice(0, 10).map((a, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 px-3 py-2 bg-orange-50 border border-orange-100 rounded-lg"
                  >
            ...
            <div className="px-6 pb-5 flex gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  document
                    .querySelector('[data-section="project-activities"]')
          ...
          <div className="mx-4 max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden animate-[scaleIn_0.25s_ease-out]">
            ...
            <div className="px-5 py-4">
              <p className="text-base font-semibold text-gray-800">
                You&apos;ve been idle for {fmtIdle(idleSeconds)}
              </p>
              <p className="text-sm text-gray-500 mt-2 leading-relaxed">
                No mouse movement or keyboard activity has been detected. Your
                idle time is being logged and may affect your productivity
                report.
              </p>
        ...
        >
          <div className="px-2 sm:px-3 lg:px-4 py-2">
            <div className="bg-white/60 backdrop-blur-sm rounded-xl border border-gray-200/60 shadow-sm p-2 sm:p-3 xl:p-4 space-y-3 xl:space-y-4">
              {/* ── Hero Header ── */}
              <div
                className="relative overflow-hidden rounded-lg bg-gradient-to-br from-[#64126D] via-[#7a1785] to-[#9333ea] px-4 py-3 xl:px-6 xl:py-4 shadow-md"
                style={{ perspective: '800px', transformStyle: 'preserve-3d' }}
              >
                <div
                  className="absolute -top-10 -right-10 w-32 h-32 rounded-full bg-white/[0.07] blur-sm animate-[blobFloat1_8s_ease-in-out_infinite]"
                  style={{ transform: 'translateZ(20px)' }}
                />
                <div
                  className="absolute -bottom-12 -left-8 w-28 h-28 rounded-full bg-white/[0.06] blur-sm animate-[blobFloat2_10s_ease-in-out_infinite]"
                  style={{ transform: 'translateZ(15px)' }}
                />
                <div
                  className="absolute top-1/2 right-1/4 w-16 h-16 rounded-full bg-purple-300/10 blur-[2px] animate-[blobFloat3_12s_ease-in-out_infinite]"
                  style={{ transform: 'translateZ(30px)' }}
                />
                <div
                  className="absolute -top-4 left-1/3 w-20 h-20 rounded-full bg-pink-300/[0.07] blur-sm animate-[blobFloat4_9s_ease-in-out_infinite]"
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-3 xl:p-4">
                ...
                <div className="grid grid-cols-2 gap-2.5 xl:gap-3 sm:grid-cols-3 lg:grid-cols-7">
                  ...
                  {(() => {
                    const totalMins =
                      liveTotalTime.hrs * 60 + liveTotalTime.mins;
                    const isComplete = totalMins >= EXPECTED_WORK_MINS;
                    const hasLogin = !!attendance.loginTime;
                    return (
                      <div
                        className={`group relative rounded-xl border p-3 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 overflow-hidden ${
                          !hasLogin
                      ...
                      >
                        <div
                          className={`absolute top-0 right-0 w-20 h-20 rounded-full -translate-x-4 -translate-y-4 blur-lg group-hover:scale-125 transition-transform duration-500 ${
                            !hasLogin
                              ? 'bg-indigo-200/30'
                              : isComplete
                                ? 'bg-green-200/30'
                        ...
                        <div className="relative">
                          <div className="flex items-center justify-between mb-2">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 shadow-sm ${
                                !hasLogin
                                  ? 'bg-gray-200 text-gray-500'
                                  : isComplete
                                    ? 'bg-gradient-to-br from-green-500 to-emerald-600 text-white shadow-green-200 group-hover:shadow-green-300 group-hover:shadow-md'
                          ...
                          {hasLogin && !isComplete && (
                            <p className="text-[9px] text-red-500 mt-0.5 font-semibold">
                              {Math.floor(
                                ...
                                ? `${Math.floor((EXPECTED_WORK_MINS - totalMins) / 60)}h `
                                : ''}
                              {(EXPECTED_WORK_MINS - totalMins) % 60}m remaining
                            </p>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="group relative rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 via-white to-sky-50 p-3 hover:shadow-lg hover:border-blue-400 hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
                    ...
                    <div className="relative">
                      <div className="flex items-center justify-between mb-2">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 shadow-sm ${attendance.daysPresent > 0 ? 'bg-gradient-to-br from-blue-500 to-sky-600 text-white shadow-blue-200 group-hover:shadow-blue-300 group-hover:shadow-md' : 'bg-gray-200 text-gray-500'}`}
                        >
                      ...
                      {attendance.daysInMonth > 0 && (
                        <div className="mt-2 h-1.5 rounded-full bg-blue-100 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.round((attendance.daysPresent / attendance.daysInMonth) * 100)}%`,
                            }}
                          />
                        </div>
                      )}
                  ...
                  <div className="group relative rounded-xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-yellow-50 p-3 hover:shadow-lg hover:border-amber-400 hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
                    ...
                    <div className="relative">
                      <div className="flex items-center justify-between mb-2">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 shadow-sm ${attendance.leaves.used > 0 ? 'bg-gradient-to-br from-amber-500 to-yellow-600 text-white shadow-amber-200 group-hover:shadow-amber-300 group-hover:shadow-md' : 'bg-gray-200 text-gray-500'}`}
                        >
                  ...
                  <div className="group relative rounded-xl border border-orange-200 bg-gradient-to-br from-orange-50 via-white to-amber-50 p-3 hover:shadow-lg hover:border-orange-400 hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
                    ...
                    <div className="relative">
                      <div className="flex items-center justify-between mb-2">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 shadow-sm ${attendance.overtimeHours > 0 ? 'bg-gradient-to-br from-orange-500 to-amber-600 text-white shadow-orange-200 group-hover:shadow-orange-300 group-hover:shadow-md' : 'bg-gray-200 text-gray-500'}`}
                        >
                  ...
                  {(() => {
                    ...
                    const effectiveIdleSecs = attendance.logoutTime
                      ? 0
                      : idleSeconds;
                    const totalIdleMin = attendance.logoutTime
                      ? attendance.idleTime || 0
                      : Math.floor(cumulativeIdleSeconds / 60) +
                        (attendance.idleTime || 0);
                    return (
                      <div
                        className={`group relative rounded-xl border p-3 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 overflow-hidden ${
                          effectiveIdleSecs > 1800
                            ? 'border-red-300 bg-gradient-to-br from-red-50 via-white to-rose-50 ring-1 ring-red-200'
                            ...
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 shadow-sm ${
                                effectiveIdleSecs > 1800
                                  ...
                                  : effectiveIdleSecs > 300
                                    ? 'bg-gradient-to-br from-amber-500 to-yellow-600 text-white shadow-amber-200'
              ...
              {moduleAccess.dashboard &&
                moduleAssignments.upcomingDeadlines.length > 0 && (
                  <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl shadow-sm border border-amber-200/80 p-3 xl:p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h3 className="text-xs xl:text-sm font-semibold text-amber-800 uppercase tracking-wide">
                        Upcoming Deadlines (Next 7 Days)
                      </h3>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 xl:gap-3">
                      {moduleAssignments.upcomingDeadlines
                        .slice(0, 6)
                        .map((deadline, idx) => (
                          <div
                            key={idx}
                            className="bg-white rounded-md p-2 border border-amber-100"
                          >
