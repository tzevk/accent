...
export default function DocumentMasterPage() {
	...
	const filtered = useMemo(() => {
		...
		return docs.filter(
			(d) =>
				...
				(d.doc_key || '').toLowerCase().includes(t) ||
	...
	const handleSubmit = async (e) => {
		e.preventDefault();
		if (!form.doc_key.trim() || !form.name.trim())
			return alert('Key and name are required');
		try {
			const payload = {
			...
			if (editingId) {
				await fetchJSON('/api/document-master', {
					method: 'PUT',
					...
					<div className="px-6 lg:px-8 xl:px-12 2xl:px-16 pt-16 pb-8 space-y-6 max-w-[1800px] mx-auto w-full">
						<header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
							<div>
								<h1 className="text-2xl font-bold text-black">
									Document Master Catalogue
								</h1>
								<p className="text-sm text-gray-600">
									Define document types and metadata used across the app.
								</p>
							</div>
						...
						<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
							<aside className="bg-white border border-gray-200 rounded-lg shadow-sm p-5 space-y-4">
								...
								{loading ? (
									...
									<div className="text-sm text-gray-500 border border-dashed border-gray-200 rounded-lg px-3 py-5 text-center">
										No documents yet. Create one to begin.
									</div>
								) : (
									<div className="space-y-3">
										{filtered.map((d) => (
											<div
												key={d.id}
												className={`border rounded-lg px-4 py-3 transition-colors ${selectedId === d.id ? 'border-[#7F2487] bg-[#7F2487]/10' : 'border-gray-200 hover:bg-gray-50'}`}
											>
												<div className="flex items-start justify-between">
													...
													>
														...
														<span className="mt-2 inline-flex items-center rounded-full bg-gray-100 px-2 py-1 text-[11px] text-black">
															Status: {d.status}
														</span>
													</button>
												</div>
												<div className="mt-3 flex items-center gap-2 text-xs">
													<button
														...
														className="px-3 py-1.5 rounded border border-gray-300 text-black hover:bg-gray-100"
													>
														Edit
													</button>
													<button
										...
										<form onSubmit={handleSubmit} className="space-y-4">
											<div>
												...
												<input
													...
													value={form.doc_key}
													onChange={(e) =>
														setForm((p) => ({ ...p, doc_key: e.target.value }))
													}
													required
											...
											<div className="flex justify-end gap-3">
												<button
													...
													className="px-4 py-2 text-sm rounded-md bg-gray-100 text-black hover:bg-gray-200"
												>
													Cancel
												</button>
												<button
							...
							<section className="lg:col-span-2 space-y-4">
								{loading ? (
									<div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8 text-center text-sm text-gray-700">
										Loading…
									</div>
								) : !selectedId ? (
									<div className="bg-white border border-gray-200 rounded-lg shadow-sm p-8 text-center text-sm text-gray-700">
