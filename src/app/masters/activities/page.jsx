...
export default function ActivityMasterPage() {
	...
	const [newSubActivityManhours, setNewSubActivityManhours] = useState('');
	...
	const filteredSubActivities = useMemo(() => {
		...
		if (!subActivitySearch) return subActivities;
		const term = subActivitySearch.toLowerCase();
		return subActivities.filter((sa) => sa.name.toLowerCase().includes(term));
	}, [selectedActivity, subActivitySearch]);
	...
	const handleAddActivity = async () => {
		...
		try {
			...
			const parsed =
				newActivityManhours && newActivityManhours.trim() !== ''
					? parseFloat(newActivityManhours)
					: 0;
			const manhours = isNaN(parsed) ? 0 : parsed;
			console.log('Adding activity with manhours:', {
				newActivityManhours,
				parsed,
				manhours,
			});
	...
	const handleBulkAddActivities = async () => {
		...
		const manhoursPerActivity =
			totalManhours > 0 && activityCount > 0
		...
		if (failCount === 0) {
			showSuccess(
				`${successCount} activities added successfully (${manhoursPerActivity}h each)`
			);
		} else {
	...
	const handleDeleteDiscipline = async (disciplineId) => {
		...
		try {
			const res = await fetch(`/api/activity-master?id=${disciplineId}`, {
				method: 'DELETE',
			});
			if (res.ok) {
				await fetchData();
	...
	const handleDeleteAllActivities = async () => {
		...
		if (
			!confirm(
				`Delete all ${activityCount} activities from "${selectedDiscipline?.function_name}"? This action cannot be undone.`
			)
		)
			return;
	...
	const handleUpdateActivity = async () => {
		...
		try {
			...
			const manhours =
				rawValue === '' || rawValue === undefined || isNaN(parsed) ? 0 : parsed;
	...
	const handleAddSubActivity = async () => {
		if (!newSubActivityName.trim()) {
			setError('Please enter a sub-activity name');
			return;
		}
		...
		try {
			const parsed =
				newSubActivityManhours && newSubActivityManhours.trim() !== ''
					? parseFloat(newSubActivityManhours)
					: 0;
			const manhours = isNaN(parsed) ? 0 : parsed;
			...
			if (res.ok) {
				...
				setNewSubActivityManhours('');
				showSuccess('Sub-activity added successfully');
			} else {
				const data = await res.json();
				setError(data.error || 'Failed to add sub-activity');
			}
		} catch (err) {
			console.error('Failed to add sub-activity', err);
			setError('Failed to add sub-activity');
		} finally {
			setLoading(false);
		}
	};
	...
	const handleDeleteSubActivity = async (subActivityId) => {
		...
		try {
			const res = await fetch(
				`/api/activity-master/subactivities?id=${subActivityId}`,
				{ method: 'DELETE' }
			);
			if (res.ok) {
				await fetchData();
				showSuccess('Sub-activity deleted');
			}
		} catch (err) {
	...
	const handleUpdateSubActivity = async () => {
		...
		setLoading(true);
		try {
			const rawValue = editingSubActivity.manhours;
			const parsed = parseFloat(rawValue);
			const manhours =
				rawValue === '' || rawValue === undefined || isNaN(parsed) ? 0 : parsed;
			...
			const payload = {
				...
				name: editingSubActivity.name?.trim() || undefined,
				default_manhours: manhours,
			};
			...
			if (res.ok) {
				...
				showSuccess('Sub-activity updated');
			} else {
				const data = await res.json();
				setError(data.error || 'Failed to update sub-activity');
			}
		} catch (err) {
	...
	// Count totals
	const totalActivities = disciplines.reduce(
				...
				<div className="px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 py-8 pt-20 max-w-[1800px] mx-auto">
					...
					<div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
						...
						<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex items-center gap-3">
							...
							<div>
								<div className="text-2xl font-bold text-gray-900">
									{
										disciplines.filter((d) => (d.activities?.length || 0) > 0)
											.length
									}
								</div>
					...
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
							...
							<div className="p-4 border-b border-gray-200 bg-purple-50/50">
								<div className="flex gap-2">
									<input
										...
										onKeyDown={(e) =>
											e.key === 'Enter' && handleAddDiscipline()
										}
									/>
									<button
								...
								{filteredDisciplines.length === 0 ? (
									<div className="p-8 text-center text-gray-500">
										...
										<p className="text-sm mt-1">
											Add your first discipline above
										</p>
									</div>
								) : (
											...
											<div
												...
												className={`group flex items-center gap-3 px-4 py-3 cursor-pointer transition-all ${
													selectedDisciplineId === discipline.id
														? 'bg-purple-100 border-2 border-[#7F2487]'
						...
						<div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
							...
							{/* Add Activity Form */}
							<div
								className={`p-4 border-b border-gray-200 ${selectedDisciplineId ? 'bg-blue-50/50' : 'bg-gray-50'}`}
							>
										...
										<div className="flex items-center justify-between">
											<p className="text-xs text-gray-500">
												...
												<span className="font-medium text-[#7F2487]">
													{selectedDiscipline?.function_name}
												</span>
											</p>
														...
														<p className="text-xs text-gray-500 mt-2">
															...
															{bulkManhours &&
																` × ${(
																	...
																	2
																)}h each = ${parseFloat(bulkManhours).toFixed(2)}h total`}
														</p>
													</div>
												)}
							...
							<div className="max-h-[500px] overflow-y-auto">
								{!selectedDisciplineId ? (
									<div className="p-8 text-center text-gray-500">
										...
										<p className="text-sm mt-1">
											Choose a discipline from the left panel to view and manage
											its activities
										</p>
									</div>
									...
									<div className="p-8 text-center text-gray-500">
										...
										<p className="text-sm mt-1">
											Add activities to &ldquo;
											{selectedDiscipline?.function_name}&rdquo;
										</p>
									</div>
								) : (
										...
										<tbody className="divide-y divide-gray-100">
											{filteredActivities.map((activity, index) => (
												<tr
													...
													className={`group cursor-pointer transition-colors ${selectedActivityId === activity.id ? 'bg-blue-50/70 border-2 border-[#4472C4]' : 'hover:bg-gray-50'}`}
												>
						...
						{/* THIRD PANEL - Sub-Activities */}
						<div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
							{/* Panel Header */}
							<div className="bg-gradient-to-r from-[#10B981] to-[#059669] px-5 py-4">
								<div className="flex items-center justify-between">
									<div className="flex items-center gap-3">
										<div className="p-2 bg-white/20 rounded-lg">
										...
										<div>
											...
											<p className="text-xs text-white/70">
												{selectedActivity
													? `${selectedActivity.activity_name} • ${selectedActivity.subActivities?.length || 0} sub-activities`
													: `Select an activity`}
											</p>
										</div>
									</div>
							...
							<div className="px-4 pt-3 pb-2 border-b border-gray-100">
								<div className="relative">
									...
									<input
										...
										className="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#10B981] focus:border-transparent"
									/>
								</div>
							</div>
							...
							<div
								className={`p-4 border-b border-gray-200 ${selectedActivityId ? 'bg-emerald-50/50' : 'bg-gray-50'}`}
							>
								{selectedActivityId ? (
									<div className="space-y-3">
										<div className="flex items-center justify-between">
										...
										<div className="flex gap-2">
											<input
												...
												className="flex-1 px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#10B981] focus:border-transparent"
												onKeyDown={(e) =>
											...
											<input
												type="text"
												inputMode="decimal"
												value={newSubActivityManhours}
												onChange={(e) => {
													const val = e.target.value;
													if (val === '' || /^\d*\.?\d*$/.test(val)) {
														setNewSubActivityManhours(val);
													}
												}}
												placeholder="Hours"
												className="w-24 px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#10B981] focus:border-transparent"
												onKeyDown={(e) =>
													e.key === 'Enter' && handleAddSubActivity()
												}
											/>
											<button
												...
												disabled={loading || !newSubActivityName.trim()}
												className="px-4 py-2.5 bg-[#10B981] text-white rounded-lg text-sm font-medium hover:bg-[#059669] disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-colors whitespace-nowrap"
											>
												<PlusIcon className="w-4 h-4" />
												Add
											</button>
									...
									<div className="text-center py-2 text-sm text-gray-500">
										<span className="inline-flex items-center gap-2">
											...
											Select an activity to add sub-activities
										</span>
									</div>
								)}
							</div>
							...
							<div className="max-h-[500px] overflow-y-auto">
								{!selectedActivityId ? (
									<div className="p-8 text-center text-gray-500">
										<CheckIcon className="h-12 w-12 mx-auto mb-3 text-gray-300" />
										<p className="font-medium">Select an activity</p>
										<p className="text-sm mt-1">
											Choose an activity from the middle panel to view and
											manage its sub-activities
										</p>
									</div>
								) : filteredSubActivities.length === 0 ? (
									<div className="p-8 text-center text-gray-500">
										<CheckIcon className="h-12 w-12 mx-auto mb-3 text-gray-300" />
										<p className="font-medium">No sub-activities yet</p>
										<p className="text-sm mt-1">
											Add sub-activities to &ldquo;
											{selectedActivity?.activity_name}&rdquo;
										</p>
									</div>
								) : (
									<table className="w-full">
										...
										<tbody className="divide-y divide-gray-100">
											{filteredSubActivities.map((subActivity, index) => (
												...
												>
													...
													<td className="px-4 py-3">
														{editingSubActivity?.id === subActivity.id ? (
															<input
																type="text"
																...
																className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-[#10B981] focus:border-transparent"
																autoFocus
																onKeyDown={(e) => {
																	if (e.key === 'Enter')
													...
													<td className="px-4 py-3 text-right">
														{editingSubActivity?.id === subActivity.id ? (
															<input
																...
																className="w-20 px-2 py-1 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-[#10B981] focus:border-transparent text-right"
																onKeyDown={(e) => {
																	if (e.key === 'Enter')
																		handleUpdateSubActivity();
																	if (e.key === 'Escape')
															...
															<span className="text-sm font-medium text-gray-700">
																{subActivity.defaultManhours
																	? `${parseFloat(subActivity.defaultManhours).toFixed(2)} hrs`
																	: '—'}
															</span>
														)}
													</td>
														...
														{editingSubActivity?.id === subActivity.id ? (
															...
															<div className="flex justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
																<button
																	...
																	className="p-1.5 text-gray-400 hover:text-[#10B981] hover:bg-emerald-50 rounded"
