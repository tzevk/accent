			...
			<div className="px-6 py-6">
					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-sm text-gray-500">
								{stats.total} holidays | {stats.upcoming} upcoming
							</p>
						</div>
					</div>
					<div className="flex items-center gap-3">
						...
						<div className="flex bg-gray-100 rounded-lg p-1">
							<button
								...
								className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all ${viewMode === 'calendar' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
							>
							...
							<button
								...
								className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all ${viewMode === 'list' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
							>
				...
				<div className="flex items-center gap-2 mb-4 overflow-x-auto pb-2">
					<button
						...
						className={`px-3 py-1.5 text-xs font-medium rounded-full whitespace-nowrap transition-all ${filterType === 'all' ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}
					>
						All ({stats.total})
					</button>
					{Object.entries(HOLIDAY_TYPES).map(([key, type]) => {
						...
						return (
							<button
								...
								className={`px-3 py-1.5 text-xs font-medium rounded-full whitespace-nowrap transition-all flex items-center gap-1.5 ${filterType === key ? `${type.bg} ${type.text} border ${type.border}` : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}
							>
						...
						<div className="xl:col-span-3">
							<div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
								<div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50">
									<button
										onClick={() => setFocusedMonth((m) => (m > 0 ? m - 1 : 11))}
										className="p-2 hover:bg-gray-200 rounded-lg"
									>
										<ChevronLeftIcon className="w-5 h-5 text-gray-600" />
									</button>
									<h3 className="text-lg font-semibold text-gray-900">
										{MONTHS[focusedMonth]} {selectedYear}
									</h3>
									<button
										onClick={() => setFocusedMonth((m) => (m < 11 ? m + 1 : 0))}
										className="p-2 hover:bg-gray-200 rounded-lg"
									>
								...
								<div className="p-6">
									...
									<div className="grid grid-cols-7 gap-2">
										{calendarDays.map((day, idx) => {
											...
											const typeInfo = holiday
												? HOLIDAY_TYPES[holiday.type] || HOLIDAY_TYPES.national
												: null;
											const TypeIcon = typeInfo?.Icon;
											const dayIsToday = isToday(day);
											const isSunday =
												day &&
												new Date(selectedYear, focusedMonth, day).getDay() ===
													0;
											return (
												<div
													key={idx}
													className={`min-h-[80px] p-2 rounded-lg transition-all ${day ? 'hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-200' : ''} ${holiday ? `${typeInfo.bg} border ${typeInfo.border}` : ''} ${dayIsToday ? 'ring-2 ring-blue-400' : ''} ${isSunday && !holiday ? 'bg-red-50/50' : ''}`}
													onClick={() => {
														if (day && !holiday) {
															setFormData((f) => ({
																...f,
																date: `${selectedYear}-${String(focusedMonth + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`,
															}));
															setShowModal(true);
														} else if (holiday) {
															handleEdit(holiday);
												...
												>
													{day && (
														<>
															<span
																className={`text-sm font-semibold ${dayIsToday ? 'text-blue-600' : holiday ? typeInfo.text : isSunday ? 'text-red-500' : 'text-gray-700'}`}
															>
																{day}
															</span>
															{holiday && (
																<div className="mt-1">
																	<div className="flex items-center gap-1">
																		<TypeIcon
																			className={`w-3 h-3 ${typeInfo.text}`}
																		/>
																		<p
																			className={`text-[11px] font-medium ${typeInfo.text} leading-tight truncate`}
																		>
																			{holiday.name}
																		</p>
																	</div>
										...
										{MONTHS.map((m, i) => {
											...
											return (
												<button
													...
													className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${focusedMonth === i ? 'bg-gray-900 text-white' : count > 0 ? 'bg-white border border-gray-200 text-gray-700 hover:bg-gray-100' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100'}`}
												>
													{m.slice(0, 3)}
													{count > 0 && (
							...
							<div className="bg-white rounded-xl border border-gray-200 p-4">
								<div className="flex items-center justify-between mb-4">
									<h3 className="text-sm font-semibold text-gray-900">
										{MONTHS[focusedMonth]} Holidays
									</h3>
									<span className="text-xs text-gray-400">
										{holidaysByMonth[focusedMonth].length}
									</span>
								</div>
									...
									<div className="text-center py-6">
										...
										<button
											onClick={() => {
												setFormData((f) => ({
													...f,
													date: `${selectedYear}-${String(focusedMonth + 1).padStart(2, '0')}-01`,
												}));
												openAddModal();
											}}
										...
										{holidaysByMonth[focusedMonth]
											...
											.map((holiday) => {
												...
												return (
													<div
														key={holiday.id}
														className={`p-3 rounded-lg border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all ${isPast ? 'opacity-60' : ''}`}
													>
														<div className="flex items-start gap-3">
															<div
																className={`flex-shrink-0 w-10 h-10 rounded-lg ${typeInfo.bg} flex flex-col items-center justify-center`}
															>
															...
															<div className="flex-1 min-w-0">
																...
																<div className="flex items-center gap-1 mt-0.5">
																	...
																	/>
																	<span
																		className={`text-[10px] ${typeInfo.text}`}
																	>
																		{typeInfo.label}
																	</span>
																	{holiday.is_optional && (
																		<span className="px-1 py-0.5 text-[9px] font-medium bg-amber-100 text-amber-700 rounded ml-1">
																			Optional
																		</span>
																	)}
																</div>
										...
										return (
											<div key={key} className="flex items-center gap-2">
												...
												<div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
													<div
														className={`h-full ${type.dot} rounded-full`}
														style={{ width: `${pct}%` }}
													/>
												</div>
												...
												return (
													...
													>
														<td className="px-4 py-3">
															<div className="flex items-center gap-3">
																<div
																	className={`w-10 h-10 rounded-lg ${typeInfo.bg} flex flex-col items-center justify-center`}
																>
																	<span className="text-xs font-bold text-gray-700">
																		{date.getDate()}
																	</span>
														...
														<td className="px-4 py-3">
															<span
																className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full ${typeInfo.bg} ${typeInfo.text} border ${typeInfo.border}`}
															>
																<TypeIcon className="w-3.5 h-3.5" />
																{typeInfo.label}
															</span>
										...
										{Object.entries(HOLIDAY_TYPES).map(([key, type]) => {
											...
											return (
												<button
													...
													className={`flex items-center justify-center gap-1 px-2 py-2 rounded-lg border text-xs transition-all ${formData.type === key ? `${type.bg} ${type.text} ${type.border}` : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
												>
													<TypeIcon className="w-3.5 h-3.5" />
