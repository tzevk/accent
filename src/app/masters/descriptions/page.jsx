...
export default function DescriptionMaster() {
	...
	const fetchDescriptions = async (searchValue) => {
		try {
			setLoading(true);
			const search =
				typeof searchValue !== 'undefined' ? searchValue : debouncedSearchTerm;
			const response = await fetch('/api/masters/descriptions');
			...
			if (result.success) {
				...
				if (search) {
					filtered = filtered.filter((item) =>
						item.description_name?.toLowerCase().includes(search.toLowerCase())
					);
				}
	...
	const handleEdit = (description) => {
		setFormData({
			...
			is_active: isActive(description.is_active),
		});
		setEditingId(description.id);
		setActiveTab('add');
	};

	const handleDelete = async (id, descriptionName) => {
		if (!confirm(`Are you sure you want to delete "${descriptionName}"?`)) {
			return;
		}

		try {
			const response = await fetch(`/api/masters/descriptions?id=${id}`, {
				method: 'DELETE',
			});
			const result = await response.json();
			...
			if (result.success) {
				...
				alert('Error deleting description: ' + result.error);
			}
		} catch (error) {
			console.error('Error:', error);
				...
				<div className="bg-white rounded-lg border border-gray-200 p-6">
					<div className="flex items-center">
						...
						<div className="ml-4">
							...
							<p className="text-2xl font-semibold text-gray-900">
								{descriptions.filter((item) => isActive(item.is_active)).length}
							</p>
						</div>
					</div>
				</div>
							...
							<input
								...
								onKeyDown={(e) => {
									if (e.key === 'Enter') {
										...
										setDebouncedSearchTerm(searchTerm.trim());
										fetchDescriptions(searchTerm.trim());
									}
								}}
				...
				{loading ? (
					...
					<div className="p-8 text-center">
						...
						<p className="mt-1 text-sm text-gray-500">
							Get started by adding a new description.
						</p>
					</div>
				) : (
	...
	const renderAddDescriptionForm = () => (
		<div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
			<div className="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
				...
				<div className="flex space-x-3">
					...
					>
						...
						<span>{submitting ? 'Saving...' : 'Save Description'}</span>
					</button>
				</div>
			</div>

			<form id="add-description-form" onSubmit={handleSubmit} className="p-6">
				<div className="space-y-6">
					<div>
