...
export default function Vendors() {
	...
	const handleDelete = async (id, vendorName) => {
		if (
			window.confirm(`Are you sure you want to delete vendor "${vendorName}"?`)
		) {
			try {
				const result = await fetchJSON(`/api/vendors/${id}`, {
					method: 'DELETE',
				});
						...
						<div className="mb-6 flex items-center justify-between">
							<div>
								...
								<p className="text-gray-600">
									Manage suppliers, subcontractors, and service providers
								</p>
							</div>
							<button
										...
										{loading ? (
											...
											filteredVendors.map((vendor) => (
												<tr key={vendor.id} className="hover:bg-gray-50">
													...
													<td className="px-6 py-4 whitespace-nowrap">
														<span
															className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(vendor.status)}`}
														>
													...
													<td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
														<div className="flex space-x-2">
															...
															<button
																onClick={() =>
																	handleDelete(vendor.id, vendor.vendor_name)
																}
