...
export default function Leads() {
	...
	const handleFormSubmit = async (e) => {
		...
		try {
			...
			if (result.success) {
				...
				if (
					newLeadId &&
					formData.first_followup_date &&
					formData.first_followup_description
				) {
					try {
						...
						console.warn(
							'Lead created but follow-up creation failed:',
							followUpError
						);
					}
				}
	...
	const handleCompanyChange = (e) => {
		...
		setFormData((prev) => ({
			...
			phone: selectedCompany.phone || selectedCompany.mobile_number || '',
			inquiry_email: selectedCompany.email || '',
		}));
	};
	...
	const convertToProposal = async (lead) => {
		...
		if (!confirm(`Convert lead for "${lead.company_name}" to a proposal?`))
			return;
									...
									>
										...
										<td className="w-32 px-4 py-3 whitespace-nowrap">
											<div className="space-y-1">
												<span
													className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(lead.enquiry_status)}`}
												>
													{lead.enquiry_status}
												</span>
												<div
													className={`text-xs font-medium ${getPriorityColor(lead.priority)}`}
												>
			...
			{leads.length > 0 && (
				<div className="flex items-center justify-between">
					<div className="flex items-center text-sm text-gray-700">
						...
						{Math.min(currentPage * 20, stats.total_leads || 0)} of{' '}
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-gray-600 mt-1">
								Create a new lead and set up initial follow-up
							</p>
						</div>
