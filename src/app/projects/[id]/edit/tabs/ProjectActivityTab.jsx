							...
							{projectActivities.length} activities across{' '}
							{
								Object.keys(
									projectActivities.reduce((acc, a) => {
										acc[a.discipline || a.function_name || 'Manual'] = true;
										return acc;
									}, {})
								).length
							}{' '}
				...
				<div className="flex items-center gap-3">
					<div className="flex items-center gap-2 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
						...
						<span className="text-xs text-blue-700 font-medium">
							Plan:{' '}
							{projectActivities
								.reduce((sum, a) => sum + getActivityTotalPlanned(a), 0)
								.toFixed(1)}
							h
						</span>
					</div>
					<div className="flex items-center gap-2 bg-green-50 px-3 py-1.5 rounded-lg border border-green-100">
						...
						<span className="text-xs text-green-700 font-medium">
							Actual:{' '}
							{projectActivities
								.reduce((sum, a) => sum + getActivityTotalActual(a), 0)
								.toFixed(1)}
							h
						</span>
					</div>
				...
				{showActivitySelector && functions.length > 0 && (
					<div className="border border-gray-200 rounded-lg bg-white text-sm">
						...
						<div className="max-h-48 overflow-y-auto divide-y divide-gray-100">
							{functions.map((func) => {
								const filteredActs = (func.activities || []).filter(
									(act) =>
										!activitySelectorSearch ||
										act.activity_name
											?.toLowerCase()
											.includes(activitySelectorSearch.toLowerCase())
								);
								if (activitySelectorSearch && filteredActs.length === 0)
									return null;
								const allSelected =
									filteredActs.length > 0 &&
									filteredActs.every(
										(act) => selectedActivitiesForAdd[`${func.id}|${act.id}`]
									);
								...
								return (
									<div key={func.id}>
										<div
											...
											onClick={() =>
												filteredActs.length > 0 &&
												toggleAllActivitiesInDiscipline(func.id, filteredActs)
											}
										>
											{filteredActs.length > 0 ? (
												<input
										...
										{filteredActs.length > 0 && (
											<div className="grid grid-cols-3 gap-x-1 px-2 py-1">
												{filteredActs.map((activity) => {
													const isAlreadyAdded = projectActivities.some(
														(pa) =>
															String(pa.id) === String(activity.id) &&
															pa.type === 'activity'
													);
													const isSelected =
														selectedActivitiesForAdd[
													...
													return (
														...
														>
															<input
																...
																onChange={() =>
																	!isAlreadyAdded &&
																	toggleActivitySelection(func.id, activity.id)
																}
					...
					<div>
						<div className="overflow-visible">
							<table className="w-full text-xs border-collapse">
								...
								<tbody>
									{(() => {
										const groupedActivities = projectActivities.reduce(
											(acc, act) => {
												const disciplineKey =
													act.discipline ||
													act.function_name ||
										...
										return Object.entries(groupedActivities).map(
											(
												...
												const disciplinePlanned = activitiesInDiscipline.reduce(
													(sum, a) => sum + getActivityTotalPlanned(a),
													0
												);
												const disciplineActual = activitiesInDiscipline.reduce(
												...
												return (
													...
													>
																...
																<div className="flex items-center justify-between">
																	...
																	<div className="flex items-center gap-3 text-[11px]">
																		<span className="text-blue-700 font-medium">
																			Plan: {disciplinePlanned.toFixed(1)}h
																		</span>
																		<span className="text-green-700 font-medium">
																			Actual: {disciplineActual.toFixed(1)}h
																		</span>
																	</div>
																</div>
															</td>
														...
														{activitiesInDiscipline.map((act) => {
															...
															const subActivities =
																matchingActivity?.subActivities || [];
															...
															const discipline =
																act.discipline || act.function_name || '';
																	...
																	{Array.from({
																		...
																		if (odUserId) {
																			...
																			const user = userList.find(
																				(u) => String(u.id) === String(odUserId)
																			);
																			const teamMember = !user
																				? projectTeamMembers.find(
																						(m) =>
																							String(m.id) === String(odUserId)
																					)
																				: null;
																			name = user
																				? user.full_name ||
