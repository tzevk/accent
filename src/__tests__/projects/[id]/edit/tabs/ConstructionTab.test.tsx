...
describe('ConstructionTab', () => {
	it('renders all form controls with correct initial values', () => {
		...
		const { container } = render(
			<ConstructionTab form={form} handleChange={vi.fn()} />
		);

		// Mobilization Date
		const dateInput = container.querySelector(
			'input[name="mobilization_date"]'
		);
		...
		const textarea = container.querySelector(
			'textarea[name="construction_progress"]'
		);
	...
	it('calls handleChange when mobilization date changes', async () => {
		...
		const { container } = render(
			<ConstructionTab form={baseForm} handleChange={handleChange} />
		);
