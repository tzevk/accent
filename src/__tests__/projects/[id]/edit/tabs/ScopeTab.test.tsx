...
describe('ScopeTab', () => {
    it('renders without crashing and displays empty states', () => {
        ...
        expect(screen.getByText('No original scope defined yet')).toBeInTheDocument()
        expect(screen.getByText('No additional scope items added yet')).toBeInTheDocument()
        
        // Verify badge
        expect(screen.getByText('No Original Scope')).toBeInTheDocument()
        
        // Overview section should not be present
        expect(screen.queryByText('Complete Scope Overview')).not.toBeInTheDocument()
    })
    ...
    it('renders original scope from scope_of_work and displays correct badges', () => {
        ...
        expect(screen.getAllByText('Original design scope contents')).toHaveLength(2)
