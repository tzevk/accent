...
describe('QuotationTab', () => {
	...
	it('renders empty message when proposal_id is missing', () => {
		render(
			<QuotationTab form={{}} canEditQuotations={true} quotationData={{}} />
		);

		expect(
			screen.getByText(/No proposal linked to this project/i)
		).toBeInTheDocument();
	});
	...
	it('renders quotation details correctly when proposal_id exists', () => {
		...
		expect(
			screen.getAllByText((content) => content.includes('11,800')).length
		).toBeGreaterThan(0);
	});
	...
	it('opens quotation edit page when Edit & Download Quotation is clicked', async () => {
		...
		const editBtn = screen.getByRole('button', {
			name: /Edit & Download Quotation/i,
		});
