...
describe('CloseoutTab', () => {
    it('renders CloseoutTab form inputs with correct values', () => {
        ...
        const { container } = render(<CloseoutTab form={form} handleChange={vi.fn()} />)

        const select = container.querySelector('select[name="final_documentation_status"]')
        expect(select).toHaveValue('Reviewed')

        const profitInput = container.querySelector('input[name="actual_profit_loss"]')
        ...
        const feedback = container.querySelector('textarea[name="client_feedback"]')
    ...
    it('triggers handleChange when inputs change', async () => {
        ...
        const { container } = render(<CloseoutTab form={baseForm} handleChange={handleChange} />)
