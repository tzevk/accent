...
function startCleanup() {
	...
	cleanupTimer = setInterval(() => {
		...
		for (const [key, entry] of store.entries()) {
			...
			if (
				entry.windowStart + RATE_LIMITS.api.windowMs * 2 < now &&
		...
		if (deleted > 0) {
			console.log(
				`[RateLimiter] Cleaned up ${deleted} expired entries, ${store.size} remaining`
			);
		}
	}, CLEANUP_INTERVAL);
...
export function checkRateLimit(request, category) {
	...
	if (entry.count > config.maxRequests) {
		...
		console.warn(
			`[RateLimiter] IP ${ip} blocked for ${cat} - ${entry.count} requests in window`
		);
	...
	for (const [key, entry] of store.entries()) {
		...
		if (entry.blockedUntil && entry.blockedUntil > now) {
			...
			stats.blockedIPs.push({
				ip,
				category: cat,
				unblockAt: new Date(entry.blockedUntil).toISOString(),
			});
		}
	}
