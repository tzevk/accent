...
export default function PriceBreakupEditor({ value, onChange }) {
	...
	const normalize = (arr) =>
		(arr || []).map((r) => ({ ...r, id: r.id ?? ++nextId.current }));
	const [rows, setRows] = useState(
		value && Array.isArray(value)
			? normalize(value)
			: [{ id: ++nextId.current, label: 'Item 1', quantity: 1, unitPrice: 0 }]
	);
	...
	const updateRow = (idx, field, val) => {
		const next = rows.map((r, i) =>
			i === idx
				? { ...r, [field]: field === 'label' ? val : Number(val || 0) }
				: r
		);
		setRows(next);
	};
			...
			<div className="space-y-2">
				{rows.map((r, idx) => (
					<div key={r.id} className="grid grid-cols-12 gap-2 items-center">
						<input
							className="col-span-5 px-2 py-1 border border-gray-300 rounded-md"
							value={r.label}
							onChange={(e) => updateRow(idx, 'label', e.target.value)}
						/>
						<input
							className="col-span-2 px-2 py-1 border border-gray-300 rounded-md"
							type="number"
							min="0"
							value={r.quantity}
