...
export default function RecentLeadsTable() {
  ...
  const fetchLeadsPage = useCallback(
    async (page) => {
      try {
        ...
        if (data.success) {
          const rawLeads = Array.isArray(data.data?.leads)
            ? data.data.leads
            : [];
          ...
          setLeadsTotal(
            Number(data.data.pagination?.total || normalized.length || 0)
          );
  ...
  const getStatusColor = (status) => {
    ...
    if (s.includes('lost') || s.includes('rejected'))
      return 'bg-red-100 text-red-800';
    if (s.includes('discussion') || s.includes('pending'))
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Latest inquiries and opportunities
        </p>
      </div>
            ...
            <tr>
              ...
              >
                <div className="flex items-center gap-1">
                  Date
                  <ChevronDownIcon
                    className={`w-4 h-4 transition-transform ${sortBy === 'created_at' && sortOrder === 'asc' ? 'rotate-180' : ''}`}
                  />
                </div>
              </th>
              ...
              >
                <div className="flex items-center gap-1">
                  Status
                  <ChevronDownIcon
                    className={`w-4 h-4 transition-transform ${sortBy === 'enquiry_status' && sortOrder === 'asc' ? 'rotate-180' : ''}`}
                  />
                </div>
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
            ...
            {leadsList.length === 0 ? (
              ...
              leadsList.map((lead) => (
                <tr key={lead.id || lead.lead_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(lead.enquiry_status)}`}
                    >
      ...
      {leadsTotal > rowsPerPage && (
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
          <div className="text-sm text-gray-700">
            Showing {(leadsPage - 1) * rowsPerPage + 1} to{' '}
            {Math.min(leadsPage * rowsPerPage, leadsTotal)} of {leadsTotal}{' '}
            leads
          </div>
