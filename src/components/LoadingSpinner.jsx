	...
	const content = (
		<div className="text-center">
			...
			<div className="relative inline-flex mb-6">
				<div
					className={`${s.spinner} rounded-full border-4 border-gray-200`}
				></div>
				<div
					className={`absolute top-0 left-0 ${s.spinner} rounded-full border-4 border-transparent border-t-violet-500 border-r-violet-500 animate-spin`}
				></div>
				<div className="absolute inset-0 flex items-center justify-center">
					<svg
						className={`${s.icon} text-violet-500`}
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
...
export function InlineSpinner({ message = 'Loading...', className = '' }) {
	...
	if (className) {
		return (
			<div
				className={`rounded-full border-2 border-gray-200 border-t-current animate-spin ${className}`}
			></div>
		);
	}
