/**
 * Payroll Slip PDF rendering, shared by the admin bulk export and an
 * employee's own download (issue #247).
 *
 * One landscape A4 page per Payroll Slip; `renderSlip` draws a whole slip
 * inside one outer box. Both routes render through `renderSlipsPdf`, so the
 * document an employee downloads is the one an admin would export for them.
 */
...
/** The slips as a PDF buffer — one page each, landscape A4. */
export function renderSlipsPdf(slips) {
	const doc = new jsPDF({
