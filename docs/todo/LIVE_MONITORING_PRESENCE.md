...
3. **Low-permission users never appeared online (prod report).** The tracker endpoint guarded with `ensurePermission(PROFILE, READ)`, but users with empty permission arrays (e.g. role-less employees like "Ashish Hadkar": `role_id` NULL, `permissions: []`) get 403 → their heartbeats silently never arrived (client swallows errors) → no presence row, and only `login`/`logout` rows in the audit log. Fix: `track-activity` now authenticates with `getCurrentUser` — any logged-in user may report their own activity; the id always comes from the session, never the payload, so there's no escalation. Verified live: empty-perm user's heartbeat lands and writes presence.
4. **Live-monitoring default filter** changed from `online`-only to **Online & Idle** (`'active'` = `online` + `idle`), per product request — people reading a page (client-idle) are no longer hidden from the default view.
