...
| **P0.3 `priority` column drift (6 missing columns on `projects`)**                                                                                      | `migrations/20260824120000_fix_projects_column_drift.js:1` (new)                                                                                                                                                                                                                                   | Adds `budget`, `priority`, `progress`, `notes`, `activities`, `disciplines` to `projects` (each `information_schema`-guarded/idempotent, types mirrored from `proposals`) + backfill; fixes `ER_BAD_FIELD_ERROR` on `POST /api/projects` and proposal→project convert                  |
...
### P0.3 `priority` column drift (just caused migrate failure) ✅ FIXED 2026-08-24
...
- **Fix (decided: add columns — the UI uses them):** `migrations/20260824120000_fix_projects_column_drift.js`. Live schema check revealed the drift was wider than `priority`: the hardcoded INSERTs (`POST /api/projects`, proposal→project convert, legacy fallback in `proposals/[id]`) reference **6** missing columns — `budget`, `priority`, `progress`, `notes`, `activities`, `disciplines` — so fixing only `priority` would still fail on `Unknown column 'budget'`. Migration adds all six, each guarded via `information_schema` (idempotent), types mirrored from `proposals` (convert copies these values verbatim): `budget DECIMAL(15,2) NULL`, `priority VARCHAR(20) NOT NULL DEFAULT 'MEDIUM'` after `type` (VARCHAR not ENUM — accepts legacy case variants; UI uses LOW/MEDIUM/HIGH and filters case-insensitively), `progress DECIMAL(5,2) DEFAULT '0.00'`, `notes TEXT`, `activities/disciplines LONGTEXT`; backfills `priority='MEDIUM'`. Verified: migrate/rollback/re-apply cycle clean, all 30 INSERT columns resolve against live schema. Kept API code untouched — `PUT` already prunes to existing columns.

### P0.4 Rate limiter + userCache won't scale ✅ logout gap FIXED 2026-08-25 · remainder DEFERRED to P2
...
- **Partial fix 2026-08-24:** the worst manifestation is gone — `src/app/api/proposals/[id]/route.js` looked up `INFORMATION_SCHEMA` with `process.env.DB_NAME || 'accent'`, but `DB_NAME` isn't defined (pool uses `DEV_DB_NAME=accent_crm_dev_muse`), so the column set came back empty: proposal saves failed with "No updatable columns found for proposals table" and proposal→project convert silently degraded to the legacy hardcoded INSERT. All three lookups now resolve the schema from the live connection (`TABLE_SCHEMA = DATABASE()` / `SELECT DATABASE()`). Remaining: `knexfile.js`/`database.js` env-var unification.
