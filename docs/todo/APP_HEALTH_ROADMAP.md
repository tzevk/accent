...
Verification: `npm run lint` on touched files clean, `npx tsc --noEmit` pre-existing errors only, `npx vitest run src/__tests__/projects` 31/31, full suite 85/85 + sanitize tests 4/4.
...
### P0.2 Missing transaction on multi-write project update
...
- **Fix:** `await db.beginTransaction()` / `commit()` / `rollback()` via `withDb()` helper `src/utils/database.js:253`. Wrap the three statements.
