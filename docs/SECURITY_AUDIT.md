...
| SEC-19 | 🟡 Medium   | `scripts/setup-super-admin.js` (deleted 2026-08-24)                                                                                                                                            | Hardcoded default super-admin credentials (`crmadmin` / `admin123`) in committed script                                              | Resolved 2026-08-24     |
...
**Resolution (2026-08-24):** Script deleted from the repository (commit removing it also cleans related one-off payroll/debug scripts). Super admins should be created with `scripts/create_super_admin.sql` (placeholder-based, no committed secrets). If `crmadmin`/`admin123` was ever used against any environment, that account's password must be rotated.
