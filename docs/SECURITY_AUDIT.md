...
2. **SEC-02 + SEC-06 + SEC-10 (upload/storage model)** — ~~SEC-02 done 2026-08-13~~ (uploads rasterized to PNG, non-raster rejected, `/uploads/*` served with nosniff + attachment). Remaining: move files out of `public/`, serve via authenticated routes, stop embedding `entity_id` in filenames (SEC-06 + SEC-10).
