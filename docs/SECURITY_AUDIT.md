...
| SEC-02 | 🔴 Critical | `src/app/api/uploads/route.js:43-53`, `middleware.ts:17,297-308`                                                                                                                               | Stored XSS: raw SVG uploads persisted to `public/uploads/` and served inline, same-origin, no CSP/nosniff                            | Open                    |
...
| SEC-04 | 🟠 High     | `src/app/api/users/route.js:293-325`, `src/app/api/users/[id]/route.js:110-165`                                                                                                                | Privilege escalation / account takeover: `password_hash` client-writable, unvalidated `permissions`/`role_id` grants                 | Open                    |
| SEC-05 | 🟠 High     | `src/app/api/login/route.js:66-73,104-109`, `src/utils/api-permissions.js:215-218`                                                                                                             | Account deactivation is a no-op: disabled users log in, are auto-re-activated, keep API access                                       | 🟡 Partial (2026-08-11) |
| SEC-06 | 🟠 High     | `src/app/api/document-upload/route.js:39-120,123-160,201-252`, `middleware.ts:5-16,300-310`                                                                                                    | IDOR + public exposure: any authenticated user lists/reads/deletes any entity's documents; files served from `public/` with no auth  | Open                    |
| SEC-07 | 🟠 High     | `src/app/api/users/route.js:24-40,354-363`, `src/app/api/users/[id]/route.js:33-47,141-158`, `src/app/api/employees/route.js:83-88`, `src/app/api/employees/[id]/route.js:33-36`               | `password_hash` (all users) and bank/PAN/Aadhaar PII leaked via verbatim `SELECT *` responses                                        | Open                    |
...
| SEC-10 | 🟠 High     | `src/app/api/document-upload/route.js:97-99,112-115,233-234`                                                                                                                                   | Path traversal: unsanitized `entity_id` embedded in filename → arbitrary file write + unlink                                         | Open                    |
...
**Status: 🟡 Partial (2026-08-11).** The `|| authenticated` fallback in `getCurrentUser`/`_fetchUserFromDb` was removed with the SEC-01 rewrite — a deactivated user's API sessions no longer inherit active status from cookie presence. **Remaining:** the login query still filters only `isDelete = 0` (disabled users can log in) and login still runs `UPDATE ... is_active = TRUE, status = 'active'` (auto re-activation); `_fetchUserFromDb` has no `is_active`/`status` filter. Own workstream, untouched here.
...
6. **SEC-11 (rate-limit IP source) + SEC-05 (deactivation enforcement)** — trusted IP source for rate limiting; enforce `is_active` at login and in `_fetchUserFromDb`, drop the login-time auto re-activation.
